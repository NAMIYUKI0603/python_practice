...
# --- 1. 空間設計（ディレクトリと経路） ---
INPUT_TEXT = "input/input_製造業_金属製品製_金属加工_20260617_1624.txt"
...
# ★分析の解像度調整パラメータ
MIN_EDGE_WEIGHT = 7   # 最低何回ペアになったら線を結ぶか
...
# --- 3. データの読み込みと独立・不可侵の境界線認識 ---
print("テキストを読み込み、独立した事故ごとに相関関係を計算中...")
...
# 【重要改修】改行を消し去る処理を廃止。
# 抽出エンジンが出力した「改行」を事故の境界線（壁）として認識させる。
incidents = re.split(r'\n+', raw_text)

# --- 4. 形態素解析と共起（ペア）の厳密なカウント ---
t = Tokenizer()
pair_counts = {}
...
for incident in incidents:
    ...
    # 事故の記述内でさらに文ごとに分割（句点で分割）
    sentences = incident.split('。')
    for sentence in sentences:
        ...
        # 【重要】他の事故とは絶対に交ざらない。同一文内でのみ共起をカウントする。
