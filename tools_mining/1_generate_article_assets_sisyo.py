...
print(f"--- スナイパー型アセット生成エンジン（キーワードフィルター実装版）起動 [{current_time}] ---")
...
# 🔥【新規実装】災害状況テキスト内のキーワード部分一致フィルター（OR条件）
...
if record_count == 0:
    print("[終了] 条件に一致するデータ（キーワード含む）が存在しません。設定を見直してください。")
    exit()
...
print(f"\n[大成功] 災害状況キーワードが完全連動したグラフアセットが出荷されました。")
